...
const createOrder = async (req, res) => {
  try {
    ...
    if (!customerNumber)
      return res.status(400).json("Please enter a customer number");
...
const getOrderByName = async (req, res) => {
  try {
    ...
    const filter = {
      name: { $regex: name, $options: 'i' }, // Using 'i' option for case-insensitive matching
    };

    const productsByName = await product.find(filter);
